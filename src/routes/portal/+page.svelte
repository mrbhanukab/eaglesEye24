	...
	async function downloadImage() {
		try {
			const fieldId = await WebApp.getFile(
				data.aetosPath.upcoming[0]?.Image || data.aetosPath.notFixed[0]?.Image
			);
	...
	<div class="portalContainer">
		<section class="widget potralComponent portalAetos">
			...
			>
				<div class="lectureInfo">
					<div class="info">
						<h1>
							{data.aetosPath.upcoming[0]?.Title ?? data.aetosPath.notFixed[0]?.Title ?? 'No Title'}
						</h1>
						<h6>by
							<span>
															{data.aetosPath.upcoming[0]?.lecture ??
															data.aetosPath.notFixed[0]?.lecture ??
															'Unknown Lecturer'}
							</span>
						</h6>
					</div>
					<div class="dateOrLink">
						{#if data.aetosPath.upcoming[0] && new Date(data.aetosPath.upcoming[0].start) <= now && (!new Date(data.aetosPath.upcoming[0].end) || new Date(data.aetosPath.upcoming[0].end) >= now)}
							<a href="/watch/{data.aetosPath.upcoming[0].youtube}">
								<h1>▶</h1>
								<h6>Watch Live</h6>
							</a>
						{:else if data.aetosPath.upcoming[0]}
							<h1>{new Date(data.aetosPath.upcoming[0].start).getDate() ?? '--'}</h1>
							<h6>
								{new Date(data.aetosPath.upcoming[0].start).toLocaleTimeString('en-US', {
									hour: 'numeric',
									minute: 'numeric'
								}) ?? 'Not Fixed'}
							</h6>
		...
		<section class="potralComponent potralRight">
			<div class="widget aetosMindAndXploratus">
				{@render subComponent('aetosMind', 'Aetos Mind')}
				{@render subComponent('Xploratus', 'Xploratus')}
			</div>
			<header class="header">
...
{#snippet subComponent(className, title)}
