	...
	{#snippet aboveImage()}
		<div class="newTeamPageAboveImage">
			...
			<p>
				...
				>. You can add up to {numberOfMembersAllowed} members to this team. Although the system allows
				you to add team members with only the restriction of a maximum number of members,
				<b style="font-weight: bolder; color: red"
					>if you add fewer members than required by the rules and regulations of '{data.event}',
					...
					<div class="addMembers">
						...
						<div class="counter-buttons">
							<button
								...
								disabled={numberOfMembers <= 1 || dataSubmited}>-</button
							>
							<p>{numberOfMembers} / {numberOfMembersAllowed}</p>
							<button
								type="button"
								onclick={incrementMembers}
								disabled={numberOfMembers >= numberOfMembersAllowed || dataSubmited}>+</button
							>
						</div>
					</div>
				</div>
